...
export function SosWizard({ userId, demoMode = false }: Props) {
  ...
  const proceedWithCoords = useCallback(

    async (resolved: { lat: number; lng: number; accuracy: number }) => {

      setCoords(resolved);

      const zone = await resolveServiceZone(resolved.lat, resolved.lng);
      ...
      const { data: newRequestId, error: createError } = await supabase.rpc(
...

        {
          ...
          p_accuracy_m: resolved.accuracy,
  ...
  const locateAndCheckZone = useCallback(async () => {
    ...
    try {
      ...
      const resolved = await acquireDevicePosition();

      await proceedWithCoords(resolved);
  ...
  const locateFromCap = useCallback(async () => {
    ...
    try {
      ...
      const resolved = await geocodeItalianCap(capInput);

      await proceedWithCoords(resolved);

    } catch (err) {

      const message = err instanceof Error ? err.message : "CAP non valido.";
  ...
  const confirmDiagnosis = async () => {
    ...
    try {
      ...
      if (demoMode) {
        ...
        if (!coords || !diagnosis) throw new Error("Dati incompleti");
      ...
      {step === "zone" && (
        ...
        <Card>
          ...
          <CardContent className="space-y-4 p-6">
            ...
            <h2 className="text-lg font-semibold">Verifica zona</h2>

            <p className="text-sm text-muted">

              Usiamo la tua posizione per verificare che SuperMastro sia attivo nella tua area e

              per trovare mastri vicini.
            ...
            >
              ...
              {loading ? "Acquisizione posizione…" : "Usa la mia posizione"}
            ...
            <div className="space-y-3 border-t border-[var(--border)] pt-4">
              ...
              <p className="text-sm text-muted">Oppure inserisci il CAP</p>
                ...
                <Button
                  ...
                  disabled={loading || capInput.length !== 5}

                  variant="outline"

                  size="full"

                  onClick={() => void locateFromCap()}

                >

                  {loading ? "Verifica in corso…" : "Continua con CAP"}
